        ...
        <div class="testDetails">
          ...
          <div class="block">
            <div class="level-left">
              <p class="title is-4">
                Locust Config
                <a class="button" @click="isEditSimUsersModalActive = true;">
                  <span class="icon is-small">
                    <font-awesome-icon icon="edit" />
                  </span>
                </a>
            ...
            <div class="box">
              ...
              <div class="field">
                ...
                <input
                  class="input"
                  type="number"
                  v-model="numOfSimUsers"
                  placeholder="Enter a number"
                />
                <label class="label" style="padding-top:15px">Hatch Rate:</label>
                <input class="input" type="number" v-model="hatchRate" placeholder="Enter a number" />
                <label class="label" style="padding-top:15px">Run Time:</label>
                <input class="input" type="number" v-model="runTime" placeholder="Enter a number" />
              </div>
              <div class="field is-grouped">
                <div class="control">
                  <button
                    class="button is-danger"
                    @click="isEditSimUsersModalActive = false; editLocustConfig();"
                  >Update</button>
                </div>
                <div class="control">
                  <button class="button" @click="isEditSimUsersModalActive = false;">Cancel</button>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  data: function() {
    return {
      ...
      isEditSimUsersModalActive: false,
  ...
  methods: {
    ...
    editLocustConfig: function() {
      console.log("numOfSimUsers", this.numOfSimUsers);
      this.testConfig = [
        { Name: "test-config-file", Value: "testconfig.txt" },
        { Name: "clients", Value: this.numOfSimUsers },
        { Name: "hatch-rate", Value: this.hatchRate },
        { Name: "run-time", Value: this.runTime },
        { Name: "loglevel", Value: "ERROR" },
        { Name: "load-profile", Value: "[(0,0), (10m,100%), (+15m,0%)]" }
      ];
    },
    ...
    deployTest: function() {
      //current hack to deploy the first selected grid
      let gridToDeploy = this.selectedGrids[0];
      console.log("Grid to Deploy: ", gridToDeploy.Name);

      var path, body;

      path = "/api/test/" + this.test.ID + "/start";
      body = {
        GridID: gridToDeploy.ID,
        GridRegion: gridToDeploy.Region,
